                        ...
                        <tbody>
                            @foreach ($transaction as $trx)
                                <tr>
                                    ...
                                    <td class="text-center"
                                        style="vertical-align: middle; white-space: normal; font-size: 13px;">
                                        {{ $trx->BankUser->nama_pemilik }} /  {{ $trx->BankUser->nama_bank }} / {{ $trx->BankUser->nomor_rekening }}</td>
                                    <td class="text-center"
                                        style="vertical-align: middle; white-space: normal; font-size: 13px;">
                                        {{ $trx->keterangan }}</td>
                                    <td class="text-center"
                                    ...
                                    <td class="text-center"
                                        style="vertical-align: middle; white-space: normal; font-size: 13px;">
                                        <a href="{{ route('withdrawal.approve',$trx->id) }}" class="btn btn-success btn-sm"
                                            onclick="return confirm('Are you sure want to Confirm this Transaction?');">Approve</a>
                                        <a href="{{ route('withdrawal.reject',$trx->id) }}" class="btn btn-danger btn-sm"
